...
# CKUSDC Deposit Test Script using dfx CLI
# This script tests ckUSDC token deposits to the ICSI canister using the dfx identity system
...
echo -e "${ROCKET} Testing CKUSDC Deposit with ICSI Canister"
echo "============================================="
...
CKUSDC_CANISTER_ID="xevnm-gaaaa-aaaar-qafnq-cai"  # Mainnet ckUSDC
if [ "$NETWORK" = "local" ]; then
    # For local testing, you might need to deploy your own ICRC-1 token
    echo -e "${YELLOW} Warning: Using mainnet ckUSDC canister ID for local testing"
    echo "You may need to deploy a local ICRC-1 token for local testing"
fi

CKUSDC_DECIMALS=6
TRANSFER_AMOUNT=100000  # 0.1 ckUSDC in micro-units (6 decimals)
TRANSFER_FEE=10         # ckUSDC fee
MINIMUM_BALANCE=100010  # 0.100010 ckUSDC in micro-units (transfer + fee)
...
echo -e "\n${MONEY} CKUSDC Token Config:"
echo "   Canister ID: $CKUSDC_CANISTER_ID"
echo "   Symbol: CKUSDC"
echo "   Decimals: $CKUSDC_DECIMALS"
echo "   Network: $NETWORK"

# Function to convert amount from micro-units to CKUSDC for display
ckusdc_from_micro() {
...
# First, try to add a subaccount for CKUSDC token (might already exist)
dfx canister --network "$NETWORK" call "$ICSI_CANISTER_ID" addSubaccountForToken '(variant { CKUSDC })' >/dev/null 2>&1 || true
...
# Get subaccount ID for CKUSDC
SUBACCOUNT_RESULT=$(dfx canister --network "$NETWORK" call "$ICSI_CANISTER_ID" getSubaccountId "(${NONCE} : nat32, variant { CKUSDC })")
SUBACCOUNT_ID=$(echo "$SUBACCOUNT_RESULT" | sed -n 's/.*Ok = "\([^"]*\)".*/\1/p')

if [ -z "$SUBACCOUNT_ID" ]; then
    echo -e "${CROSS} Failed to get CKUSDC subaccount ID"
    exit 1
fi
...
echo -e "${CHECK} CKUSDC Deposit Address: $DEPOSIT_ADDRESS"
echo "   Subaccount ID: $SUBACCOUNT_ID"

# Get sender's CKUSDC balance
echo -e "\n${MONEY} Checking sender CKUSDC balance..."
BALANCE_RESULT=$(dfx canister --network "$NETWORK" call "$CKUSDC_CANISTER_ID" icrc1_balance_of "(record { owner = principal \"$PRINCIPAL\"; subaccount = null })")
...
SENDER_BALANCE_CKUSDC=$(ckusdc_from_micro "$SENDER_BALANCE")
echo "   Balance: $SENDER_BALANCE_CKUSDC CKUSDC"

# Check minimum balance
if [ "$SENDER_BALANCE" -lt "$MINIMUM_BALANCE" ]; then
    MINIMUM_CKUSDC=$(ckusdc_from_micro "$MINIMUM_BALANCE")
    echo -e "${CROSS} Insufficient CKUSDC balance. Need at least $MINIMUM_CKUSDC CKUSDC (including fee)"
    echo "   Current balance: $SENDER_BALANCE_CKUSDC CKUSDC"
    TRANSFER_CKUSDC=$(ckusdc_from_micro "$TRANSFER_AMOUNT")
    FEE_CKUSDC=$(ckusdc_from_micro "$TRANSFER_FEE")
    echo "   Required: $TRANSFER_CKUSDC CKUSDC for transfer + $FEE_CKUSDC CKUSDC for fee"
    exit 1
fi

# Convert subaccount hex to array format
SUBACCOUNT_ARRAY=$(hex_to_subaccount_array "$SUBACCOUNT_ID")
TRANSFER_AMOUNT_CKUSDC=$(ckusdc_from_micro "$TRANSFER_AMOUNT")

echo -e "\n${MONEY} Sending $TRANSFER_AMOUNT_CKUSDC CKUSDC to deposit address..."

# Make the ICRC-1 transfer
TRANSFER_RESULT=$(dfx canister --network "$NETWORK" call "$CKUSDC_CANISTER_ID" icrc1_transfer "
(record {
  to = record {
...
if echo "$TRANSFER_RESULT" | grep -q "Ok"; then
    ...
    if [ -n "$AVAILABLE_BALANCE" ]; then
        AVAILABLE_CKUSDC=$(ckusdc_from_micro "$AVAILABLE_BALANCE")
        echo "   Your available balance: $AVAILABLE_CKUSDC CKUSDC"
    fi
    exit 1
else
...
if echo "$TRANSACTIONS_RESULT" | grep -q "CKUSDC"; then
    echo -e "${CHECK} Found CKUSDC transactions in ICSI canister"
    
    # Get recent CKUSDC transactions
    echo -e "\n${CHART} Recent CKUSDC transactions:"
    echo "$TRANSACTIONS_RESULT" | grep -A 10 -B 5 "CKUSDC" | head -20
else
    echo -e "${YELLOW} No CKUSDC transactions found yet. Transaction may still be processing."
fi

echo -e "\n${CHECK} CKUSDC deposit test completed!"
