...
async function getDepositAddresses() {
  try {
    ...
    const tokens = await getRegisteredTokens(agent, [
    ...
    for (const [tokenPrincipal, tokenName] of tokenList) {
      ...
      try {
        const result = await addSubaccountForToken(agent, [
          Principal.fromText(USER_VAULT_CANISTER_ID),
          tokenPrincipal,
        ]);
      ...
      const icrcAccountResult = await getIcrcAccount(agent, [
        Principal.fromText(USER_VAULT_CANISTER_ID),
        Number(subaccountId),
      ]);
      ...
      const txCountResult = await getTransactionsCount(agent, [
        Principal.fromText(USER_VAULT_CANISTER_ID),
        Number(subaccountId),
      ]);
